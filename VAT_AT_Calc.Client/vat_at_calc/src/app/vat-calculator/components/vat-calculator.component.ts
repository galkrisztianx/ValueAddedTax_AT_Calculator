...
export class ValueAddedTaxCalculatorComponent {
  ...
  @ViewChildren(MatRadioGroup) vatRateRadioControl:
    | QueryList<MatRadioGroup>
    | undefined;
  ...
  protected readonly valueAddedTaxAmounts =
    signal<ValueAddedTaxAmountsInterface>({
      net: null,
      gross: null,
  ...
  constructor(
    private valueAddedTaxCalculatorService: ValueAddedTaxCalculatorService
  ) {}

  onCalcRadioButtonChange(event: MatRadioChange) {
    const selectedValueAddedTaxRate = event.value as number;
    this.valueAddedTaxAmounts.update((oldValue) => ({
      ...oldValue,
      valueAddedTaxRate: selectedValueAddedTaxRate,
    }));
  }
  ...
  onSubmitCalculatorForm() {
    ...
    if (this.validateCalculatorForm()) {
      this.valueAddedTaxCalculatorService
        .calculateValueAddedTaxAmounts(this.valueAddedTaxAmounts())
        .pipe(first())
